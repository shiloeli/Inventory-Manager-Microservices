...
connection.connect(function(err) {
    ...
    var sql = "CREATE TABLE cities (id INT PRIMARY KEY, city_name VARCHAR(255), city_symbol INT, toddlers INT, children INT, graduates INT, adults INT, middle_age INT, seniors INT, type VARCHAR(255))";
    connection.query(sql, function (err, result) {
      if (err) throw err;
...
.then(function (resultCity) {
  ...
  console.log(resultCity[2])
  values = []
  for(var i=0; i<1000; i++)
    values.push([cities[i]._id, cities[i].שם_ישוב_לועזי.trim(), cities[i].סמל_ישוב, ageCities[i].גיל_0_5, ageCities[i].גיל_6_18, 
    ageCities[i].גיל_19_45, ageCities[i].גיל_46_55, ageCities[i].גיל_56_64, ageCities[i].גיל_65_פלוס, "מעורב"])  
   mySQL.insertData(values)
}),


insertData: function(res) {
  var sql = `INSERT INTO cities (id, city_name, city_symbol, toddlers, children, graduates, adults, middle_age, seniors, type) VALUES ?`;
